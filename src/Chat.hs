...
sendRequestToChat apiKey queryMsg = do
    ...
    case result of
      Left failure -> print failure >> (pure $ Left (show failure))
      Right success -> do
        print $ chrChoices success
        pure $ Right $ T.concat $ map extractContentFromChatChoise $ chrChoices success
